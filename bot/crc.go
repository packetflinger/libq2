package bot

// Quake II's move-command checksum, ported from id's qcommon/crc.c and
// qcommon/common.c, which Yamagi carries unchanged.
//
// A vanilla-protocol server VERIFIES the byte a client puts in front of its
// clc_move block: SV_ExecuteClientMessage recomputes COM_BlockSequenceCRCByte
// over the block and IGNORES THE REST OF THE PACKET when it disagrees, without
// telling anybody.  Q2PRO dropped that check long ago, so a client that makes
// the byte up works there and is simply never moved by Yamagi Quake II, id's
// own q2ded or r1q2 -- it connects, it spawns, it receives frames, and every
// usercmd it sends is discarded.
//

// crcTable is id's crctable.  id ships it as 256 literals, but it is the
// textbook CCITT table for polynomial 0x1021, non-reflected, so it is computed
// here instead: a table that can be derived cannot be mis-transcribed.
var crcTable = func() (t [256]uint16) {
	for i := range t {
		c := uint16(i) << 8
		for j := 0; j < 8; j++ {
			if c&0x8000 != 0 {
				c = c<<1 ^ 0x1021
			} else {
				c <<= 1
			}
		}
		t[i] = c
	}
	return
}()

// chkTable is id's chktbl, and it is arbitrary data -- nothing derives it, so
// these 960 bytes are the one thing here that has to be carried.
//
// id declares it [1024] and initialises 960 values; C zero-fills the rest and
// the sequence index reaches into that tail, so the zeroes are part of the
// algorithm rather than an omission.  A sized Go array literal zero-fills the
// same way, which makes this the same construct as id's rather than a
// transcription of its consequences.
var chkTable = [1024]byte{
	,
}

// blockSequenceCRCByte is id's COM_BlockSequenceCRCByte: a CCITT CRC over at
// most the first 60 bytes of the block, salted with four bytes taken from
// chkTable at an offset derived from the outgoing sequence number, folded
// together with the byte sum.
//
// id copies the block and the salt into a scratch buffer and then walks that
// buffer twice, once for the CRC and once for the sum.  Running both over the
// two pieces in place is the same arithmetic without the copy.
func blockSequenceCRCByte(base []byte, sequence int) byte {
	if sequence < 0 {
		return 0
	}
	if len(base) > 60 {
		base = base[:60]
	}
	salt := chkTable[sequence%(len(chkTable)-4):][:4]

	crc := uint16(0xffff) // CRC_INIT_VALUE
	sum := 0
	for _, chunk := range [2][]byte{base, salt} {
		for _, b := range chunk {
			crc = crc<<8 ^ crcTable[byte(crc>>8)^b]
			sum += int(b)
		}
	}
	return byte(int(crc) ^ sum)
}
