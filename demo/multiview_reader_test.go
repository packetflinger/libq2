...
func TestMvdParseServerData(t *testing.T) {
	tests := []struct {
		...
		{
			name: "test0",
			data: "24 25000000 da07 b5218e5e 6f70656e74646d00 14 00",
			want: &pb.MvdServerData{
				Protocol:         2010,
				Identity:         1586373045,
	...
	for _, tc := range tests {
		t.Run(tc.name, func(t *testing.T) {
			inbytes := strings.ReplaceAll(tc.data, " ", "")
			in, err := hex.DecodeString(inbytes)
			if err != nil {
			...
			if err != nil {
				t.Errorf("ParseServerData(%v) error: %v", inbytes, err)
			}
			if diff := cmp.Diff(got, tc.want, protocmp.Transform(), protocmp.IgnoreFields(parser.demo.Remap)); diff != "" {
				t.Errorf("ParseServerData(%v) = \n%v,\nwant \n%v\n", &buf, got, tc.want)
			}

			writer := NewMVD2Writer(nil)
			out := writer.MarshalServerData(got)
			outstr := hex.EncodeToString(out.Data)
			if outstr != inbytes {
				t.Errorf("MarshalServerData(%v) = \n%v,\nwant \n%v\n", got, outstr, inbytes)
			}
		})
	}
}
